...
__main__ () {
	...
	## $ dpkg -L pekwm | grep xsession | grep desktop
	## /usr/share/xsessions/pekwm.desktop
	## $ grep '^Exec=' /usr/share/xsessions/pekwm.desktop
	## $ grep '^Exec=' $(dpkg -L pekwm | grep xsession | grep desktop)
	## Exec=pekwm
	## $ which pekwm
	## /usr/bin/pekwm

	## run pekwm
	#DISPLAY=:100 /usr/bin/pekwm
	DISPLAY=:100 pekwm
