...
__main__ () {
	...
	## $ dpkg -L icewm | grep xsession | grep desktop
	## /usr/share/xsessions/icewm-session.desktop
	## $ grep '^Exec=' /usr/share/xsessions/icewm-session.desktop
	## $ grep '^Exec=' $(dpkg -L icewm | grep xsession | grep desktop)
	## Exec=icewm-session
	## $ which icewm-session
	## /usr/bin/icewm-session

	## run icewm-session
	#DISPLAY=:100 /usr/bin/icewm-session
	DISPLAY=:100 icewm-session
