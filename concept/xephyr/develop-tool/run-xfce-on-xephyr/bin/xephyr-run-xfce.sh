...
__main__ () {
	...
	## $ dpkg -L xfce4-session | grep desktop | grep xsessions
	## /usr/share/xsessions/xfce.desktop
	## $ grep '^Exec=' /usr/share/xsessions/xfce.desktop
	## $ grep '^Exec=' $(dpkg -L xfce4-session | grep desktop | grep xsessions)
	## Exec=startxfce4
	## $ which startxfce4
	## /usr/bin/startxfce4

	## run startxfce4
	#DISPLAY=:100 /usr/bin/startxfce4
	DISPLAY=:100 startxfce4
