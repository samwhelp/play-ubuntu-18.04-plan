...
__main__ () {
	...
	## $ dpkg -L herbstluftwm | grep xsession | grep desktop
	## /usr/share/xsessions/herbstluftwm.desktop
	## $ grep '^Exec=' /usr/share/xsessions/herbstluftwm.desktop
	## $ grep '^Exec=' $(dpkg -L herbstluftwm | grep xsession | grep desktop)
	## Exec=herbstluftwm
	## $ which herbstluftwm
	## /usr/bin/herbstluftwm

	## run herbstluftwm
	#DISPLAY=:100 /usr/bin/herbstluftwm
