...
__main__ () {
	...
	## $ dpkg -L qtile | grep xsession | grep desktop
	## /usr/share/xsessions/qtile.desktop
	## $ grep '^Exec=' /usr/share/xsessions/qtile.desktop
	## $ grep '^Exec=' $(dpkg -L qtile | grep xsession | grep desktop)
	## Exec=qtile
	## $ which qtile
	## /usr/bin/qtile

	## run qtile
	#DISPLAY=:100 /usr/bin/qtile
	DISPLAY=:100 qtile
