...
__main__ () {
	...
	## $ dpkg -L spectrwm | grep xsession | grep desktop
	## /usr/share/xsessions/spectrwm.desktop
	## $ grep '^Exec=' /usr/share/xsessions/spectrwm.desktop
	## $ grep '^Exec=' $(dpkg -L spectrwm | grep xsession | grep desktop)
	## Exec=spectrwm
	## $ which spectrwm
	## /usr/bin/spectrwm

	## run spectrwm
	#DISPLAY=:100 /usr/bin/spectrwm
