...
__main__ () {
	...
	## $ dpkg -L lxqt-session | grep xsession | grep desktop
	## /usr/share/xsessions/lxqt.desktop
	## $ grep '^Exec=' /usr/share/xsessions/lxqt.desktop
	## $ grep '^Exec=' $(dpkg -L lxqt-session | grep xsession | grep desktop)
	## Exec=startlxqt
	## $ which startlxqt
	## /usr/bin/startlxqt

	## run startlxqt
	#DISPLAY=:100 /usr/bin/startlxqt
	DISPLAY=:100 startlxqt
