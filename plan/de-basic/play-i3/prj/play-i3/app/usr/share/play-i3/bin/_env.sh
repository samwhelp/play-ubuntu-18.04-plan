...
base_env_profile_list () {
	base_env_dir_prepare
	ls "$THE_ENV_DIR_PATH" -1 ## ls /usr/share/play-i3/var/env -1
}
...
base_env_profile_use_set () {
	...
	echo "$1" > "$THE_ENV_USER_USE_FILE_PATH" ## ~/.config/play-i3/env/.use
...
is_plan_env_use_file_not_exist () {
	! test -f $THE_ENV_USE_FILE_PATH ## /usr/share/play-i3/env/.use
}

is_user_env_use_file_not_exist () {
	! test -f $THE_ENV_USER_USE_FILE_PATH ## ~/.config/play-i3/env/.use
}
...
plan_env_profile_use_get () {
	...
	cat "$THE_ENV_USE_FILE_PATH" ## cat /usr/share/play-i3/var/env/.use
}
...
user_env_profile_use_get () {
	...
	cat "$THE_ENV_USER_USE_FILE_PATH" ## cat ~/.config/play-i3/env/.use
}

plan_env_profile_dir_path () {
	echo "$THE_ENV_DIR_PATH/$1" ## /usr/share/play-i3/var/env/$1
}

user_env_profile_dir_path () {
	echo "$THE_ENV_USER_DIR_PATH/$1" ## ~/.config/play-i3/env/$1
}


plan_env_default_val_file_path () {
	echo "$THE_ENV_DEFAULT_PROFILE_DIR_PATH/$1" ## /usr/share/play-i3/var/env/default/$1
}

plan_env_val_file_path () {
	echo "$THE_ENV_DIR_PATH/$1/$2" ## /usr/share/play-i3/var/env/$1/$2
}

user_env_val_file_path () {
	echo "$THE_ENV_USER_DIR_PATH/$1/$2" ## ~/.config/play-i3/env/$1/$2
}
...
base_env_val_file_path () {
	...
	val_file_path=$(user_env_val_file_path "$1" "$2") ## ~/.config/play-i3/env/$1/$2
	base_env_try_val_file_path "$val_file_path" && return 0

	val_file_path=$(plan_env_val_file_path "$1" "$2") ## /usr/share/play-i3/var/env/$1/$2
	base_env_try_val_file_path "$val_file_path" && return 0

	val_file_path=$(plan_env_default_val_file_path "$2") ## /usr/share/play-i3/var/env/default/$2
	base_env_try_val_file_path "$val_file_path" && return 0
...
base_env_set () {
	...
	plan_val_file_path=$(plan_env_val_file_path "$profile" "$key") ## ~/.config/play-i3/env/$1/$2
	...
	user_profile_dir_path=$(user_env_profile_dir_path "$profile") # ~/.config/play-i3/env/$profile

	mkdir -p "$user_profile_dir_path"

	user_val_file_path=$(user_env_val_file_path "$profile" "$key") # ~/.config/play-i3/env/$profile/$key
